...
class Game:
        ...
        for c in range(NUM_COLUMNS):
            for r in range(NUM_ROWS):
                if self.board[r][c] == 1:
                    ...
                    c * SQUARE_SIZE + SQUARE_SIZE // 2,
                    HEIGHT - (r + 2) * SQUARE_SIZE + SQUARE_SIZE // 2), RADIUS)
    ...
    def run(self):
        ...
        while not self.game_over:
            ...
            pygame.display.update((0, HEIGHT - SQUARE_SIZE, WIDTH, SQUARE_SIZE))

            if 0. not in self.board:
                label = font1.render("It's a tie!", 1, WHITE)
                pygame.draw.rect(screen, BLACK,
                                 (0, HEIGHT - SQUARE_SIZE, WIDTH, SQUARE_SIZE))
                screen.blit(label, (25, HEIGHT - 75))
                pygame.display.update(
                    (0, HEIGHT - SQUARE_SIZE, WIDTH, SQUARE_SIZE))
                self.game_over = True
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    try:
                        pygame.draw.rect(screen, BLACK,
                                         (0, HEIGHT - SQUARE_SIZE, WIDTH,
                                          SQUARE_SIZE))
                        screen.blit(label, (45, HEIGHT - 75))
                    except NameError:
                        pass
                    self.draw_board()
                    if self.game_over:
